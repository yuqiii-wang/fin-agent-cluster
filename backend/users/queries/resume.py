"""Service logic for resuming a cancelled or paused query from the last LangGraph checkpoint."""
...
async def resume_query(thread_id: str) -> QueryResponse:
    ...
    # Both paused and cancelled/failed use run_resume_async (input=None):
    # pause_query now cancels the asyncio task directly (like cancel_query),
    # so the checkpoint may be mid-execution rather than at an interrupt boundary.
    # run_resume_async re-runs the interrupted node from scratch, which is safe.
