...
async def notify(
    ...
    *,
    ...
    """
    # A per-invocation nonce makes the ack_key unique across graph re-runs
    # (e.g. re-explore with the same node_id) so that old ack_confirmed events
    # replayed from Centrifugo channel history don't pollute confirmedAckKeysRef.
    nonce = uuid.uuid4().hex[:8]
    ack_key = f"{dedup_key or f'node:{node_id}:{event}'}:{nonce}"
    # Include ack_key in the payload so the frontend uses it directly
    # instead of computing node_id:status (which would miss the nonce).
