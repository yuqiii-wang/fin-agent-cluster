...
from backend.graph.agents.task_keys import STREAM_RUNNER_THROUGHPUT
...
async def run_throughput_task(
    ...
    """
    from backend.graph.agents.streamer.tasks.dispatch import dispatch_throughput_ingest  # noqa: PLC0415
    ...
    try:
        produced, stop_reason, ingest_ms = await dispatch_throughput_ingest(
            ...
            task_key=STREAM_RUNNER_THROUGHPUT,
            total_tokens=total_tokens,
            timeout_secs=float(timeout_secs),
        )
    except (asyncio.CancelledError, TaskCancelledSignal):
        elapsed_ms = int((time.monotonic() - t0_node) * 1000)
        await cancel_task(thread_id, pub_task_id, STREAM_RUNNER_THROUGHPUT)
        await finish_node_execution(node_execution_id, {"cancelled": True}, elapsed_ms)
        raise asyncio.CancelledError()
    except Exception as exc:
        elapsed_ms = int((time.monotonic() - t0_node) * 1000)
        logger.exception(
            "[throughput_task] ingest error stream_id=%s thread_id=%s: %s",
            stream_id, thread_id, exc,
        )
        await fail_task(
            thread_id, pub_task_id, STREAM_RUNNER_THROUGHPUT,
            str(exc), error_code=STREAM_PUBLISH_FAILED,
        )
        await finish_node_execution(node_execution_id, {"error": str(exc)[:500]}, elapsed_ms)
        raise

    # Ingest complete -- Centrifugo is now delivering all written tokens.
    # Transition to digesting phase AFTER ingest so the phase accurately
    # reflects "pure delivery" rather than "writing in progress".
    ...
    logger.info(
        "[throughput_task] done produced=%d stop_reason=%s pub_ms=%d tps=%.1f stream_id=%s thread_id=%s",
