...
async def dispatch_throughput_ingest(
    ...
    Throughput completion trigger: all tokens written (token-bounded).
    Uses ``async_result.get()`` in an executor thread since the ingest
    completes quickly and the result is needed to emit the ``digesting`` phase.

    Args:
        thread_id:    LangGraph thread UUID.
        stream_id:    Celery ingest run UUID.
        node_id:      Node execution UUID.
        pub_task_id:  Pre-created task row ID.
