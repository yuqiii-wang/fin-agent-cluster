...
from backend.graph.agents.streamer.errors import SCHED_STREAM_STATE_MISSING
...
#: 2.0 s absorbs the browser HTTP/1.1 6-connection-per-origin stagger that
#: spreads 200 requests over ~1.5 s across 4 FastAPI instances.  Increasing
#: this from 0.5 s → 2.0 s collapses all streams into a single fanout task
#: instead of 7 fragmented batches, eliminating the late-joiner timeout bug.
RENDEZVOUS_WINDOW_SECS: float = 2.0

#: After the initial dispatch, poll Redis every this many seconds for
#: streams that registered after the rendezvous window closed ("late joiners").
#: Late joiners occur when 50+ requests spread across 4 FastAPI instances take
#: longer than RENDEZVOUS_WINDOW_SECS to reach dispatch_scheduled_ingest.
LATE_JOIN_INTERVAL_SECS: float = 1.0

#: Multiplier applied to timeout_secs to compute the coordinator's max lifetime.
#: Must match the BLPOP multiplier in dispatch_scheduled_ingest (3×) so the
#: coordinator stays alive long enough to dispatch any stream before its BLPOP
#: times out.  For timeout_secs=60 this gives 240 s, matching blpop_timeout.
...
#: All streams in a run should fit in as few fanout tasks as possible:
#: each task occupies one Celery worker for the full timeout_secs (60 s).
#: With 8 workers and MAX=200 the 9th batch is queued until T=60 s, which
#: exceeds the BLPOP deadline (240 s at 1000 streams, fine; but at 10 k the
#: queue depth becomes 50 tasks, and streams in batch 9+ time out).
#: Setting this to 2000 means a 10 k-stream run needs only 5 fanout tasks,
#: all dispatched within the rendezvous window.  asyncio.gather in one event
#: loop handles thousands of I/O-bound coroutines without issue.
...
async def dispatch_scheduled_ingest(
    ...
    If this is the first stream for *run_id* to arrive, this coroutine also
    spawns the coordinator ``asyncio.Task`` that collects all stream configs
    and dispatches the single fanout Celery task.

    Args:
        run_id:        Shared run UUID extracted from the frontend query string.
    ...
    Raises:
        RuntimeError: When the fanout task never signals done within the BLPOP
                      deadline (``timeout_secs * 3 + _BLPOP_BUFFER_SECS``).
    """
    ...
    # Route by stream_id — done_key is fin:stream:ingest:done:{stream_id}, so
    # sharding by stream_id is semantically correct and consistent with the
    # fanout worker's RPUSH which also routes by stream_id.
...
async def _run_rendezvous_and_dispatch(run_id: str, timeout_secs: float) -> None:
    ...
    1. Sleeps ``RENDEZVOUS_WINDOW_SECS`` (0.5 s) for the initial registrations.
    2. Dispatches a fanout for all registered streams.
    3. Enters a **late-joiner loop**: polls every ``LATE_JOIN_INTERVAL_SECS``
       (0.5 s) for streams that registered after the window closed — these are
       streams whose requests were queued by the browser or Kong and arrived
       minutes after the initial dispatch.
    4. Exits when a poll finds no new registrations, or after
       ``timeout_secs * _DISPATCH_WINDOW_MULTIPLIER + _BLPOP_BUFFER_SECS``
       seconds total (matches the BLPOP deadline in dispatch_scheduled_ingest).

    Each late-joiner batch receives an adjusted timeout so all batches finish
    close to the original deadline.
    ...
    async def _dispatch_batch(new_ids: list[str], elapsed: float) -> None:
        ...
        Streams whose Redis state is missing (pipeline race between SMEMBERS
        and HSET on shard 0) are pushed a timeout result immediately so their
        BLPOP is never left hanging.  Only successfully-configured streams are
        added to ``dispatched_ids``.
        """
        ...
        # Push timeout done_keys for streams whose state could not be read so
        # their FastAPI BLPOP is unblocked immediately rather than expiring.
        # Stream_id alone is sufficient — done_key is deterministic and shard
        # routing is by stream_id, so no state lookup is needed.
        if skipped:
            await _push_timeout_for_ids(run_id, skipped)

        if not configs:
            return
        # Always give late joiners the full timeout_secs so they can produce
        # tokens.  adjusted_timeout = timeout_secs - elapsed would give 1 s
        # for streams that arrived minutes after the initial batch, effectively
        # producing 0 tokens and confusing test metrics.
        chunks = [
        ...
        # Only mark successfully-dispatched streams as done to avoid silently
        # dropping skipped streams from future late-joiner polling rounds.
        ...
        # Log each dispatched stream_id so we can cross-reference against
        # concurrency_task logs to spot sessions that never got dispatched.
        for cfg in configs:
            reg_at = float(states.get(cfg["stream_id"], {}).get("registered_at", 0) or 0)
            logger.debug(
                "[sched_coord] stream_dispatched stream_id=%s run_id=%s registered_at=%.3f",
                cfg["stream_id"], run_id, reg_at,
            )
    ...
    try:
        ...
        # Keep polling until the BLPOP deadline (timeout_secs * 3 + buffer).
        # This matches the blpop_timeout in dispatch_scheduled_ingest so the
        # coordinator is alive for the entire window in which streams can
        # register and wait.  Streams whose frontend queues them minutes after
        # the initial dispatch (due to browser connection limits or network
        # jitter) will be picked up here instead of BLPOP-timing out.
        # Do NOT exit early on "no new joiners" — streams may arrive in
        # separate waves over the full BLPOP window.
        ...
        while True:
            ...
            # Check if any new streams registered during the sleep.
        ...
        # Safety net: push timeout results so BLPOPs are never left hanging.
...
async def _push_timeout_for_ids(
    ...
    Used by ``_dispatch_batch`` to unblock FastAPI BLPOPs for streams whose
    Redis state could not be read (pipeline race between SMEMBERS and HSET).

    The done_key is constructed directly from ``stream_id`` and sharded by
    ``stream_id`` — no state lookup required.  This ensures streams with
    permanently missing state still get their BLPOP unblocked instead of
    hanging for 240 s until the BLPOP deadline.
...
async def _push_timeout_results(run_id: str) -> None:
    ...
    Constructs done_key from stream_id directly and routes by stream_id so that
    streams with missing state still get their BLPOP unblocked.
