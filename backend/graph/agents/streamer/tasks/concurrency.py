...
from backend.graph.agents.streamer.errors import STREAM_PUBLISH_FAILED
from backend.graph.agents.streamer.tasks.models import StreamTaskResult
from backend.graph.agents.task_keys import STREAM_RUNNER_CONCURRENCY
...
from backend.sse_notifications.agent_tasks import fail_task
...
async def run_concurrency_task(
    ...
    Args:
        ...
        node_id:            Sub-graph node execution UUID.
        leaf_node_id:       Leaf-node execution UUID.
        stream_id:          Streaming session UUID (Celery ingest run).
        run_id:             Shared run UUID for coordinator dispatch.

    Returns:
        :class:`StreamTaskResult` with task ID, produced count, TPS, and summary.

    Raises:
        asyncio.CancelledError: When the task is cancelled (already cleaned up).
        Exception:              On ingest failure (already cleaned up).
    """
    from backend.graph.agents.streamer.tasks.coordinator import dispatch_scheduled_ingest  # noqa: PLC0415
    ...
    # Concurrency mode: ingest and digest are simultaneous -- tokens flow to
    # Centrifugo as they are written.  The frontend auto-transitions from
    # "ingesting" to "digesting" on the first token batch; no digesting phase
    ...
    try:
        produced, stop_reason, ingest_ms = await dispatch_scheduled_ingest(
            ...
            task_key=STREAM_RUNNER_CONCURRENCY,
        ...
        await cancel_task(thread_id, pub_task_id, STREAM_RUNNER_CONCURRENCY)
        await finish_node_execution(node_execution_id, {"cancelled": True}, elapsed_ms)
        raise asyncio.CancelledError()
    except Exception as exc:
        elapsed_ms = int((time.monotonic() - t0_node) * 1000)
        logger.exception(
            "[concurrency_task] ingest error stream_id=%s thread_id=%s: %s",
            stream_id, thread_id, exc,
        )
        await fail_task(
            thread_id, pub_task_id, STREAM_RUNNER_CONCURRENCY,
    ...
    await emit_ingest_complete(thread_id, stream_id, node_id, produced, stop_reason, ingest_ms)
    await complete_task(
        thread_id, pub_task_id, STREAM_RUNNER_CONCURRENCY,
    ...
    if stop_reason in ("completed", "stable"):
        await emit_stream_complete(thread_id, stream_id, node_id, produced, tps_val, ingest_ms)
    else:
        await emit_stream_stopped(
            thread_id, stream_id, node_id, timeout_secs, total_published=produced, ingest_ms=ingest_ms
        )
    ...
    logger.info(
        "[concurrency_task] done produced=%d stop_reason=%s pub_ms=%d tps=%.1f stream_id=%s thread_id=%s",
        produced, stop_reason, pub_ms, tps_val, stream_id, thread_id,
    )
    return result
