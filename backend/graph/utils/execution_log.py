...
async def finish_node_execution(
    ...
    Uses a single ``UPDATE … RETURNING`` round-trip so the caller can derive
    the authoritative ``ended_at_ms`` as
    ...
    Also upserts the ``fin_agents.nodes`` identity record via
    :func:`upsert_node_record` using the node type from the registry.
...
async def update_node_execution_status(
    ...
    async with factory() as session:
        ...
        row = result.fetchone()
        await session.commit()
    if row is not None and status in _TERMINAL_STATUSES and row[2] is not None:
        await upsert_node_record(
            row[2],
            row[1],
            row[0],
...
async def log_node_execution(
    ...
    Also upserts the ``fin_agents.nodes`` identity record when ``node_uuid``
    is provided.
