...
async def fetch(
    ...
    Raises:
        ValueError: When FMP returns HTTP 401/403 (carries
            :data:`~backend.resources.stats.providers.errors.STATS_FMP_AUTH_ERROR`).
        ValueError: When FMP returns an empty dataset (carries
            :data:`~backend.resources.stats.providers.errors.STATS_FMP_EMPTY`).
        ValueError: On unexpected HTTP errors (carries
            :data:`~backend.resources.stats.providers.errors.STATS_PROVIDER_ERROR`).
    """
