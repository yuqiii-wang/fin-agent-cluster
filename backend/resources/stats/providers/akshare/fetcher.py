...
from backend.resources.stats.providers.errors import STATS_AKSHARE_EMPTY, STATS_PROVIDER_ERROR
...
async def fetch(symbol: str, period: str) -> StatsRecord:
    ...
    Raises:
        ValueError: When akshare returns an empty DataFrame, carrying error
                    code :data:`~backend.resources.stats.providers.errors.STATS_AKSHARE_EMPTY`.
        ValueError: On unexpected errors from the akshare library, carrying
                    error code :data:`~backend.resources.stats.providers.errors.STATS_PROVIDER_ERROR`.
    """
