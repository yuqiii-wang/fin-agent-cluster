...
Both functions are no-ops when *task_id* is ``None`` (session-level events
such as ``"done"``) or when *event_type* has no matching step status.
"""
...
async def ack_task_step(
    ...
    Args:
        ...
        task_id:    DB task PK.  If ``None`` returns immediately.
