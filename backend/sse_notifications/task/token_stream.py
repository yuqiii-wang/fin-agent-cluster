...
by :mod:`backend.sse_notifications.task.notifications`.

Control signals (cancel / pass) are checked before each token via
:func:`backend.sse_notifications.task.control._check_signal`.
"""
...
async def stream_llm_task(
    ...
    Args:
        ...
        task_id: Primary key (UUID) of the running task.
            ...
            if token:
                ...
                await stream_token(
                    thread_id,
                    {
                        "event": "token",
                        "task_id": task_id,
                        "node_name": _node_name(task_name),
                        "task_name": task_name,
                        "data": token,
                    },
                )
...
async def stream_text_task(
    ...
    if tokens_published:
        ...
        logger.info(
            ...
            "tps=%.1f task_name=%s thread_id=%s",
            tokens_published,
