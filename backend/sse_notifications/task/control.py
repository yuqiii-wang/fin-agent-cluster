...
The streaming loops in :mod:`backend.sse_notifications.task.token_stream`
...
def signal_task_control(task_id: str, action: TaskControlAction) -> None:
    """Register a control signal for a currently-streaming task.

    The next iteration of the streaming loop for *task_id* will raise
    :class:`TaskCancelledSignal` or :class:`TaskPassSignal` and stop the
    upstream generator.

    Args:
        task_id: Primary key of the running task.
        action:    ``"cancel"`` to abort, ``"pass"`` to accept partial output.
    """
    _task_signals[task_id] = action


def _check_signal(task_id: str, parts: list[str]) -> None:
    """Consume a pending control signal and raise the matching exception.

    Args:
        task_id: Primary key of the task being streamed.
