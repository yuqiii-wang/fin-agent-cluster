...
# Params: (task_id, thread_id, node_id, node_name, task_name, description, view_type, stats_views, fencing_token)
...
_INSERT_TASK = """
    INSERT INTO fin_agents.tasks
        (task_id, thread_id, node_id, node_name, task_name, description, view_type, stats_views,
         status, created_at, updated_at, fencing_token)
    VALUES (%s, %s, %s, %s, %s, %s, %s::fin_agents.task_view_types, %s, 'running', NOW(), NOW(), %s)
...
# regardless of status. Used to reuse task_id on LangGraph @task retries so
# duplicate rows are never created in fin_agents.tasks.
# Params: (thread_id, node_id, task_name)
