...
Public export
-------------
``analyze_query`` — ``NodeTask`` instance used by ``MockQueryNode.orchestrate``.
``HANDLERS``      — dict slice ``{"mock_analyze_query": _handler}`` assembled by
...
analyze_query = NodeTask(
    name=_TASK_NAME,
    description=(
        "Mock: parse the raw user query to extract the trading intent, equity ticker "
