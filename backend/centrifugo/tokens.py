"""centrifugo.tokens — JWT token generation for Centrifugo auth.
...
Channel naming
--------------
All per-thread events travel on ``thread:{thread_id}`` (``thread`` namespace).
"""
...
def make_subscription_token(
    ...
    """Generate a Centrifugo subscription JWT for ``thread:{thread_id}``.

    Args:
        user_id:     Subject claim — identifies the subscribing user.
        thread_id:   LangGraph thread UUID; forms channel ``thread:{thread_id}``.
        secret:      Centrifugo ``token_hmac_secret_key``.
