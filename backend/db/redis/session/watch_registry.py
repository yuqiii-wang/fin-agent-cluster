...
Key:   ``watch:{thread_id}``  (Redis String, UUID string value)
TTL:   3600 s
"""
...
async def register_watch(thread_id: str, task_id: str) -> None:
    """Register *task_id* as the currently watched task for *thread_id*.

    Args:
        thread_id: LangGraph thread UUID.
        task_id: Primary key (UUID string) of the task the client has expanded.
    """
    ...
    logger.debug("[watch_registry] registered task_id=%s thread_id=%s", task_id, thread_id)
...
async def get_watched_task(thread_id: str) -> Optional[str]:
    ...
    Returns:
        The watched ``task_id`` string, or ``None`` if no task is being watched.
    """
    ...
    try:
        ...
        if val is not None:
            task_id = val if isinstance(val, str) else val.decode()
            _local_cache[thread_id] = task_id
            return task_id
    except Exception as exc:  # noqa: BLE001
