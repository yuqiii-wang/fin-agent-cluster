/**
 * LineChart — renders a close-price line chart with optional overlays.
 *
 * Layout (bottom-up):
 *   Volume histogram — bottom 20 % (when volume column present)
 *   RSI 14 line      — next 20 % (when enabled)
 *   Close price line + SMA 20 overlay — remainder
 *
...
const LineChart: React.FC<Props> = ({ dfSplit, maxHeight = 320 }) => {
  ...
  // Close price bottom margin expands when RSI is shown to make room for the RSI pane.
  const closeBottom = rsiActive ? 0.44 : 0.25;
  ...
  useEffect(() => {
    ...
    const chart = createChart(containerRef.current, {
      ...
      rightPriceScale: {
        borderColor: '#2B2F38',
        scaleMargins: { top: 0.05, bottom: closeBottom },
      },
      crosshair: { mode: 1 },
    });
    ...
    closeSeries.setData(sorted.map(({ time, close }) => ({ time, value: close })));
    ...
    if (volumeIdx !== -1) {
      ...
      const prevCloseMap = sorted.map((d, i) => (i > 0 ? sorted[i - 1].close : d.close));
      volumeSeries.setData(
        sorted.map(({ time, close, volume }, i) => ({
          time,
          value: volume,
          color: close >= prevCloseMap[i] ? 'rgba(38,166,154,0.5)' : 'rgba(239,83,80,0.5)',
        })),
      );
    }
  ...
  }, [dfSplit, chartHeight, activeOverlays, closeBottom]);
