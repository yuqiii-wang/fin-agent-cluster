 ...
 *   LineChart   → close-price line chart (lightweight-charts)
...
const LineChartView: React.FC<{ dfSplit: DfSplit; maxHeight: number }> = ({ dfSplit, maxHeight }) => {
  ...
  useEffect(() => {
    ...
    if (closeIdx !== -1) {
      const lineData = dfSplit.index
        .map((date, i) => ({ time: date as string, value: (dfSplit.data[i]?.[closeIdx] ?? 0) as number }))
...
const StatsViewer: React.FC<Props> = ({ dfSplit, activeView, maxHeight = 320 }) => {
  switch (activeView) {
    case 'DataFrame':
      return <StatsDataFrame dfSplit={dfSplit} maxHeight={maxHeight} />;
    case 'CandleStick':
      return <CandleStickChart dfSplit={dfSplit} maxHeight={maxHeight} />;
    case 'LineChart':
      return <LineChartView dfSplit={dfSplit} maxHeight={maxHeight} />;
    case 'BarChart':
