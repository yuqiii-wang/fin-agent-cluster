 ...
 *   onStarted        — captures node/stream/task IDs from stream_runner
 *   onCompleted      — captures mock_pub total_published; marks pub_status
 */

import type React from "react";
import type { SessionClosureState } from "./perfSessionState";
...
export function createPhaseHandlers(
  ...
  const onStarted = (data: unknown): void => {
    // Capture IDs emitted by the stream_runner node via extra_payload.
    const d = data as { task_id?: number; node_id?: string; leaf_node_id?: string; stream_id?: string };
    if (d.node_id || d.leaf_node_id || d.stream_id || d.task_id != null) {
      // Populate closure stream_id so terminal handlers and closeSession can
      // use it as a secondary guard against stale events from a previous run.
      if (d.stream_id) state.stream_id = d.stream_id;
      patch(thread_id, {
        task_id: d.task_id ?? null,
        node_id: d.node_id ?? null,
        leaf_node_id: d.leaf_node_id ?? null,
  ...
  const onCompleted = (data: unknown): void => {
    ...
    if (key === "stream_runner.throughput" || key === "stream_runner.concurrency") {
