...
-- Per-node user-defined skills; applied by the agent loop on each run.
-- skill_id is a UUID generated by the DB on insert.
CREATE TABLE IF NOT EXISTS fin_agents.agent_skills (
    skill_id     UUID        NOT NULL PRIMARY KEY DEFAULT gen_random_uuid(),
    thread_id    TEXT        NOT NULL REFERENCES fin_agents.user_queries (thread_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS fin_agents_agent_skills_node_id_idx ON fin_agents.agent_skills (node_id);
CREATE INDEX IF NOT EXISTS fin_agents_agent_skills_node_id_status_idx
    ON fin_agents.agent_skills (node_id, status);


-- Agent memory entries for each node execution.
-- Two flavours:
--   * Task-backed memory: task_id is set; content may be NULL (the effective
--     content is then the latest task_executions.output for that task_id).
--   * Direct memory: task_id is NULL; content must be a non-empty JSONB value.
-- name is unique within a node_id so callers can look up a memory by
-- a stable logical name in addition to memory_id.
CREATE TABLE IF NOT EXISTS fin_agents.agent_memory (
    memory_id   UUID        NOT NULL PRIMARY KEY DEFAULT gen_random_uuid(),
    thread_id   TEXT        NOT NULL REFERENCES fin_agents.user_queries (thread_id) ON DELETE CASCADE,
    node_id     TEXT        NOT NULL REFERENCES fin_agents.nodes (node_id) ON DELETE CASCADE,
    -- Optional FK to fin_agents.tasks.  When set, content may be NULL — the
    -- task's own output is the canonical memory payload.
    task_id     TEXT        REFERENCES fin_agents.tasks (task_id) ON DELETE CASCADE,
    name        TEXT        NOT NULL,
    description TEXT        NOT NULL DEFAULT '',
    -- content may be NULL only when task_id IS NOT NULL (content is derived
    -- from the referenced task's output in that case).  Otherwise content must
    -- be a non-empty JSONB value (not '{}' and not 'null'::jsonb).
    content     JSONB,
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    -- Enforce the content-is-optional-when-task-backing rule.
    CHECK (
        CASE
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_memory_node_id_name
    ON fin_agents.agent_memory (node_id, name);
CREATE INDEX IF NOT EXISTS fin_agents_agent_memory_node_id_idx
    ON fin_agents.agent_memory (node_id);
CREATE INDEX IF NOT EXISTS fin_agents_agent_memory_task_id_idx
    ON fin_agents.agent_memory (task_id)
    WHERE task_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS fin_agents_agent_memory_thread_id_idx
    ON fin_agents.agent_memory (thread_id);
