...
-- Chronological memory entries written by the agent loop during execution.
-- memory_id is a UUID generated by the DB on insert.
-- compacted_into references the compacted_summary entry that replaced a set of entries.
CREATE TABLE IF NOT EXISTS fin_agents.agent_memory (
    memory_id      UUID        NOT NULL PRIMARY KEY DEFAULT gen_random_uuid(),
    thread_id      TEXT        NOT NULL REFERENCES fin_agents.user_queries (thread_id) ON DELETE CASCADE,
    node_id        TEXT        NOT NULL REFERENCES fin_agents.nodes (node_id) ON DELETE CASCADE,
    entry_type     TEXT        NOT NULL
                               CHECK (entry_type IN ('task_result', 'tool_call', 'skill_applied',
                                                     'reasoning', 'compacted_summary')),
    content        JSONB       NOT NULL DEFAULT '{}',
    seq_num        INTEGER     NOT NULL,
    status         TEXT        NOT NULL DEFAULT 'active'
                               CHECK (status IN ('active', 'forgotten', 'compacted')),
    -- UUID of the compacted_summary entry that replaced this entry (NULL if not compacted).
    compacted_into UUID        REFERENCES fin_agents.agent_memory (memory_id) ON DELETE SET NULL,
    created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS fin_agents_agent_memory_node_id_idx
    ON fin_agents.agent_memory (node_id);
CREATE INDEX IF NOT EXISTS fin_agents_agent_memory_node_id_status_idx
